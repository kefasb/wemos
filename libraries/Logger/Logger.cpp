...
void Logger::logIt(const char* const levelTxt, const char* const text,
		va_list args) const {
	out.print(levelTxt);
	out.print(prefix);
	char buf[256];
...
void Logger::logDebug(const char* const text, ...) const {
	if (isAllowedToLogOnLevel(LogLevel::DEBUG)) {
		va_list args;
		va_start(args, text);
...
void Logger::logInfo(const char* const text, ...) const {
	if (isAllowedToLogOnLevel(LogLevel::INFO)) {
		va_list args;
		va_start(args, text);
...
void Logger::logError(const char* const text, ...) const {
	if (isAllowedToLogOnLevel(LogLevel::ERROR)) {
		va_list args;
		va_start(args, text);
